...
printHelp (void)
{
  ...
  printf
    ("\n\nSolar System 3D Sim by gen04177 - v0.001 - Running with SDL %d.%d.%d\n",
...
main (int argc, char *argv[])
{
  ...
  SDL_Window *window = SDL_CreateWindow ("sss3d v0.001",
					 width, height,
  ...
  if (!OSMesaMakeCurrent (ctx, frameBuffer, GL_UNSIGNED_BYTE, width, height))
    {
      fprintf (stderr, "Failed to activate OSMesa context.\n");
      free (frameBuffer);
      OSMesaDestroyContext (ctx);
      SDL_DestroyWindow (window);
      SDL_Quit ();
  ...
  if (padCount > 0)
    {
      controller = SDL_OpenGamepad (padIDs[0]);
    }
  SDL_free (padIDs);
  ...
  while (running)
      ...
      if (controller)
	{
	  ...
	  int startButtonState = SDL_GetGamepadButton (controller,
						       SDL_GAMEPAD_BUTTON_START);
	  int rightShoulderState = SDL_GetGamepadButton (controller,
							 SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER);
	  int buttonBState =
	    SDL_GetGamepadButton (controller, SDL_GAMEPAD_BUTTON_LABEL_A);
	  ...
	  float leftX = SDL_GetGamepadAxis (controller,
					    SDL_GAMEPAD_AXIS_LEFTX) /
	    32768.0f;
	  float leftY = SDL_GetGamepadAxis (controller,
					    SDL_GAMEPAD_AXIS_LEFTY) /
	    32768.0f;
	  float rightX = SDL_GetGamepadAxis (controller,
					     SDL_GAMEPAD_AXIS_RIGHTX) /
	    32768.0f;
	  float rightY = SDL_GetGamepadAxis (controller,
